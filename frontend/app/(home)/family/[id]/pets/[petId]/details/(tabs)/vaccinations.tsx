...
const PetVaccinations = () => {
  ...
  const { data, refetch, fetchNextPage, hasNextPage, isFetchingNextPage } = useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) => VaccinationService.getAll(pageParam, Number(petId)),
    initialPageParam: "",
    getNextPageParam: (lastPage) => lastPage.next,
  });

  const { refreshing, handleRefresh } = useQueryRefresh(() => {
    queryClient.resetQueries({ queryKey: ["pets", petId, "vaccinations"], exact: true });
    refetch();
  });
    ...
    <View style={[defaultStyles.container, { paddingTop: 16 }]}>
      <FlashList
        ...
        renderItem={({ item }) => (
          <DetailsItem
            item={item}
            api='../vaccinations/[detailId]'
            icon={<FontAwesome6 name='syringe' size={24} color='black' />}
          />
        )}
