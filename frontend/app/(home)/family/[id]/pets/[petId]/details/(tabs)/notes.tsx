...
const PetNotes = () => {
  ...
  const { data, refetch, fetchNextPage, hasNextPage, isFetchingNextPage } = useInfiniteQuery({
    ...
    queryFn: ({ pageParam }) => NoteService.getAll(pageParam, Number(petId)),
    initialPageParam: "",
    getNextPageParam: (lastPage) => lastPage.next,
  });

  const { refreshing, handleRefresh } = useQueryRefresh(() => {
    queryClient.resetQueries({ queryKey: ["pets", petId, "notes"], exact: true });
    refetch();
  });
      ...
      <FlashList
        ...
        renderItem={({ item }) => (
          <DetailsItem
            item={item}
            api='../notes/[detailId]'
            icon={<FontAwesome6 name='note-sticky' size={24} color='black' />}
          />
        )}
